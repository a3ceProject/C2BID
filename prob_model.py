...
def day_analyse(dia,time_window,Path,filt_sens=1,cluster_sens=99.7, Graph=False,kind='Int_'):
    ...
    for TW in time_window:
        ...
        path = path[:loc] + str(TW) + path[loc2:] #time-window defenition on file
        loc = str(path).find(kind) + 6
        loc2=loc+str(path)[loc:].find('.')
        path = path[:loc] + str(TW) + path[loc2:]# day defeniton on file 
        loc = path.find('day') + 3
        path = path[:loc] + str(dia) + path[loc + 1:] #day defenition in file path
        loc = path.find(kind) + 4
        path = path[:loc] + str(dia) + path[loc + 1:] #time window defenition on file path
    ...
    for window in Time:
        ...
        if Graph and dia==3: graph(f,p,dia,window) #Graph
...
def main():
    ...
    count,not_detc=zip(*Parallel(n_jobs=-1)(delayed(day_analyse)(dia,TimeWindons,path,filt_sens,cluster_sens,Graph) for dia in [1]))
